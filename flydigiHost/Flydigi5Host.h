...
#ifndef CFG_TUH_FLYDIGI_EPIN_BUFSIZE
#define CFG_TUH_FLYDIGI_EPIN_BUFSIZE 64
#endif

#ifndef CFG_TUH_FLYDIGI_EPOUT_BUFSIZE
#define CFG_TUH_FLYDIGI_EPOUT_BUFSIZE 64
#endif
...
extern const usbh_class_driver_t flydigi_driver_h;
...
{
    XINPUT_UNKNOWN = 0,
    XINPUT_STANDARD = 1   // 行为级 XInput（抽象）
} xinput_type_t;
...
typedef struct
{
    ...
    uint8_t epin_buf[CFG_TUH_FLYDIGI_EPIN_BUFSIZE];
    uint8_t epout_buf[CFG_TUH_FLYDIGI_EPOUT_BUFSIZE];

    bool     in_flight;        // 当前是否已有 IN 在路上
    uint32_t next_in_us;       // 下次允许发 IN 的时间（us）

    // 是否已收到过第一帧有效输入；为 true 才允许 SRAM/1000Hz 逻辑启动
    bool sram_armed;
